...
ii) **Pure arbitrage**: it only exists in perfect capital markets! Requires capital and posting posting collateral; people are worried about others keeping promises $\implies$ shorting is costly. 
...
**Theorem**: if rank(D) = N, there exists a unique $\phi^ast \in \mathbb{R}^M$ such that $P^T$ = $\phi^{\astD}D$, where $\phi^\ast$ = D$\theta$ for some portfolio $\theta$. 
...
Today's lecture derives a similira pricing result when a *large number of returns follow an approximate factor structure*. 
