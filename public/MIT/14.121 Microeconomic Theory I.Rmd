...
14.121 is structured in 9 Lectures:
...
We've arrived at the seventh [**lecture**](https://gabrielvoelcker.netlify.com/mit/14121lecture7.pdf) of the course!

### **8) Microfoundations for GE**

We've arrived at the eighth [**lecture**](https://gabrielvoelcker.netlify.com/mit/14121lecture8.pdf) of the course! This eighth lecture deals with Microfoundations for General Equilibrium. 

If individuals have private information about their preferences, endowments, technologies, etc, this information is too enormous to be communicated to a central planner. **Market mechanism** works by using prices to give people concise sufficient statistics allowing them to make coordinated choices and arrive at a socially optimal allocation. So what are the merits of alternative economic systems? We briefly review Hurwicz(1960) $\rightarrow$ formalization of informational role of prices(t it is impossible to have a mechanism verifying a Pareto optimal allocation without revealing a supporting Walrasian equilibrium). We have many formulations: **core**, **Ostroy's no surplus condition**, **bargaining set**, **Shapley-Shubik market games**. 

**Core**: is the set of all allocations such that no coalition(set of agents) can improve on or block the allocation (make all of its members better off) by seceding from the economy and only trading among its members. This concept doesn't mention prices! **Central result**: "core convergence" $\implies$ for economies with a large number of agents, core allocations are "approximately Walrasian".

**Blocks/improves Definition**: in an exchange economy, a **coalition** is a set S $\subset$ $\{ 1,...,I \}$. A coalition S *blocks* or *improves on* allocation x by $\hat{x}$ if $\sum_{i \in S} \hat{x}^i = \sum_{i \in S} \omega^i$, and $\forall$ i: $\hat{x}^i \succeq_i x^i$ with at least one preference strict.

**Core definition**: is the set of all allocations which cannot be improved on by any coalition. Results: *i)* in an exchange economy, every core allocation is Pareto optimal; *ii)* in an exchange economy, every Walrasian Equilibrium lies in the core. Remarks: a) Walrasian Equilibrium + Core definiton $\implies$ the core is not empty; b) in a walrasian equilibrium, each consumer acts independently **vs** in the core consumers can get together and exploit potential gains(hard to coordinate; harder without central authority).

**Core convergence**: the set of *types* of consumers is $\mathcal{I}$ = $\{ 1,...,I \}$. The *R-replica economy* $\mathcal{E}_R$ is one with R consumers of each type. Total number of consumers is RI $\rightarrow$ use this setting to construct a sequence of economies. Typical allocation in an R-replica economy is: 

\[ x = (x^{i,r}) = (\underbrace{x^{1,1},...,x^{I,1}}_\text{1st economy},..., \underbrace{x^{1,R},...,x^{I,R}}_\text{R-th economy} ) \in \mathbb{R}_+^{LRI} \]

where $x^{i,r}$ is bundle of the *r*th consumer of type *i*. Note that: $\sum_{i \in \mathcal{I}} \sum_{r=1}^R x^{i,r} = R \sum_{i \in \mathcal{I}}\omega^i$

**Equal treatment property**: suppose consumers have strictly convex preferences. If x is a core allocation in $\mathcal{E}_R$, then $x^{i,r}$ = $x^{i,r'}$  $\forall r,r'$, that is, consumers of the same type consume the same bundle. Given ET, we can treat the core allocations as vectors of fixed size LI, regardless of the replica we are concerned with. **Type allocation**: the vector ($x_1,...,x_I$) $\in \mathbb{R}_+^{LI}$, being interpreted as the ET allocations where each consumer *i* gets $x^i$. We should note that the core allocations of an R-replica economy are in $\mathbb{R}^{LIR}$, which means that we can't compare core allocations of an R replica with those of an R' replica. But ET results provides a way to doing so: we can represent in $\mathbb{R}^{LI}$ the core allocations of an R-replica economy, or an R'-replica economy, just by looking at type allocations. $C_R \subset \mathbb{R}^{LI}_+$: set of type allocations corresponding to the (equal treatment) core allocations of the R-replica economy. $W_R \subset \mathbb{R}_+^{LI}$: type allocations corresponding to the (ET) Walrasian allocations of the R-replica economy. **Obs 1**: assume coalition S has an objection against a type allocation *x* in the R-replica. Then the same is true in the R + 1 replica $\implies$ $C_{R+1} \subseteq C_R \forall R$. **Obs 2**: if a type allocation *x* is Walrasian in the R-replica, it is also Walrasian in the R+1-replica and vice-versa: $W_R = W_{R+1} = W \forall R$. **Obs 3**: by our earlier result: $W \subset C_R \forall R$. If we put the three observations together: $W \subseteq ... \subseteq C_{R+1} \subseteq C_R \subseteq ... \subseteq C_1$. 

[**Debreu-Scarf theorem**](https://www.jstor.org/stable/2525306?seq=1#page_scan_tab_contents): $\cap_R C_R = W$, which means if $x^\ast \in C_R \forall R$, then $x^\ast$ is Walrasian. 

### **9) GE with Indivisibilities**

We've arrived at the ninth and last [**lecture**](https://gabrielvoelcker.netlify.com/mit/14121lecture9.pdf) of the course! How do we model GE with indivisible goods?

- Ex: housing market is a list ($(a_k,h_k)_{k= \in\{1,...,n\}}P$) such that: $a_k$ is the set of agents; $h_n$ is the set of houses that each agent $a_k$ owns. Each agent has strict preferences over houses ($P_a$). $R_a$ is the weak-preference relation associated with $P_a$. Let P = ($P_a$)$_{a \in A}$. An allocation in a housing market can be described as a **matching** $\mu$: A $\rightarrow$ H, one-to-one housing function. House $\mu(a)$ is the assigned house of agent *a* under $\mu$. A matching is **Pareto-efficient** if there is no other matching $\mathcal{v}$ such that $\mathcal{v}(a)R_{a}\mu(a)$ for all $a \in A$ and $\mathcal{v}(a)P_a \mu(a)$ for some agent a $\in$ A. The **core** is the set of matchings $\mu$ such that there exists no coalition B $\subset$ A and a matching $\mathcal{v}$ such that: a) $\forall a \in B$, $\mathcal{v}(a) = h_\ell$ for some $a_\ell \in B$; and b) $\forall a \in B$, $\mathcal{v}(a)R_a \mu(a)$ and for some b $\in$ B, $\mathcal{v}(b)P_b \mu(b)$ ($\mathcal{v}$ weakly dominates $\mu$ through B). 

- Given the example above: a matching is **individually rational** if every agent receives a house at least as good as her initial house. i) A core mathcing is individually rational (take B all |A| singletons, one by one in the definition of core). ii) a core matching is Pareto efficient (take B = A in the definition of core). 

- But is the core non-empty in the housing market? Yes! Non-empty and unique. Pf: use Gale's top trading cycles algorithm. Steps: 1) agents point to first choice and house points to the owner.....; 2) each remaining agent keeps pointing...

- **Competitive Equilibrium**: the price mechanism also achieves core matching. Given house prices: p=($p_1,...,p_n$). A house $h_m$ is affordable for agent $a_\ell$ at p if $p_m ≤ p_\ell$(budget set). A matching $\mu$ and price vector *p* is a competitive equilibrium if $\forall a, \text{ } \mu(a)$ is the best house she can afford at prices p.

- **Theorem**: there is a competitive equilibrium matching for a housing market which is given by the core matching. Pf: slides. 

- **House allocation problem: serial dictatorship**: A **serial dictatorship** is defined through a priority ordering of agents. A priority ordering is a function f : $\{1, 2, ..., n\} \rightarrow$ *A* that is one-to-one and onto. The allocation $\varphi^f$[*P*] is found as: f(1) gets first choice, f(2) gets first choice except for the choice of f(1).... f(k) gets first choice excluding the house assigned to the house all agents before f(k) chose.

- **Theorem**: A serial dictatorship is Pareto-efficient.

- **Welfare Theorems**: we can also use the core mechanism for a house allocation problem, by simply starting with an arbitrary assigned endowment, and running the core mechanism: **Core from assigned endowments**. How are these two mechanisms related? **Theorem**: for any ordering *f*, and any matching $\mu$, simple serial dictatorship induced by *f* and core from assigned endowments $\mu$ both yield Pareto efficient matchings. Moreover, for any Pareto efficient matching $\eta$, there is a simples serial dictatorship and a core from assigned endowments that yields it. Why are these welfare theorems? Refer to slides for proof. 

- **Lottery Systems**: a lottery is a probability distribution over matchings. This is a systematic procedure to select a lottery for each problem. Ex: i) *random serial dictatorship*: Randomly select an
ordering with uniform distribution and use the induced simple serial dictatorship; ii) *core from random endowments*: Randomly select a matching (to be interpreted as initial endowment) and select the core of the induced housing market. Both mechanims produce the same outcome!

[**Theorem**](https://onlinelibrary.wiley.com/doi/pdf/10.3982/TE816): The random serial dictatorship is equivalent to the core from random endowments. That is, for each problem they choose the same lottery. 
...
- **Complete**: $\succeq$ is complete on X if $\forall x,y \in X$, either $x \succeq y$ or $y \succeq x$, or both (you have to have a preference!).

- **Reflexive**: $\succeq$ is reflexive if $\forall x,y \in X$, x $\succeq$ x.

- **Transitive**: $\succeq$ is transitive on X if x $\succeq$ y, y $\succeq$ z $\implies$ x $\succeq$ z. 

- **Monotone**: $\succeq$ is monotone if x $\in$ X and y ≥ x implies y $\succeq$ x. If x ≠ y and y ≥ x $\implies$ y $\succ$ x, then **strictly monotone** (goods are goods $\implies$ since a good is a good thing, you'll always prefer to have more of it than less of it).

- **Locally non-satiated**: if $\forall$ x and $\delta$>0, $\exists$y such that: ||y - x|| < $\delta$ and y $\succ$ x (you can always consume a little bit more of the good, not reaching the limit $\implies$ never attains a maximum).

- **Convex**: $\forall x$, y $\succeq$ x and z $\succeq$ x and z ≠ y $\implies$ that $\forall \alpha \in (0,1)$ we have: $\alpha y + (1 - \alpha)z \succeq x$ (agents prefer to diversify/moderation in all things).

- **Continuous**: it is preserved under limits: $\forall$ sequences $\{(x^n, y^n)\}^\infty_{n=1}$ with $x^n \succeq y^n$ $\forall$ n, x $\succeq$ y, where x and y are limit points of the sequence. This means we don't have "jumps" on preferences.

- **Debreu's Theorem**: if the rational preference $\succeq$ on X is continuous $\implies$ $\exists$ a continuous utility function u(x) that represents $\succeq$. 
**Revealed Preferences**: 

- **Directly revealed preferred**: if x is chosen when y could have been chosen, **x is directly revealed preferred to y**. Notation: $xR^Dy$. 

- **Local nonsatiation**: the utility function is LNS if $\forall x \in X$, and all $\varepsilon$ > 0, $\exists y \in X$ with ||y = x|| < $\varepsilon$ such that u(y) > u(x). 

### Demand Theory

- **x(p,w)**: is the **Walrasian demand correspondence**. It's the solution to the Consumer's Problem: what he chooses when maximizing utility. 4 implications:

    a) It is homogeneous of degree 0.
    
    b) compact if prices $\gg 0$. 

    c) Walras' Law: if preferences are LNS $\implies$ $\forall$(p,w) and $\forall$ $x \in x(p,w)$ we have px = w. All the wealth is spent.

    d) if u is strictly-quasiconcave, each x(p,w) contains a single bundle.
    
- **Kuhn-Tucker** necessary conditions: used to obtain the solutin to maximization/minimization problems. if *u* is continuously differentiable. If *x* $\in x(p,w)$ is a solution to the UMP, then $\exists$ a constant $\lambda$ ≥ 0 such that $\frac{du}{dx_i} = \lambda p_i \forall \in {1,2,...,L}$ with $x_i^\ast$ > 0; and $\frac{du}{dx_i} ≤ \lambda p_i \forall i \in \{ 1,2,...,L \}$ with $x_i^\ast$ = 0. The constand $\lambda$ is the "lagrange multiplier". 

- **Elasticity of demand with income**: how the demand changes when there is a unit change in income: $\eta_i = \frac{\partial x_i}{\partial w} \times \frac{w}{x_i} = \frac{\partial log x_i}{\partial log w}$.

- **Elasticity of demand with price**:  how the demand changes when there is a unit change in price: $\epsilon_{ij} = \frac{\partial x_i}{\partial p_j} \frac{p_j}{x_i} = \frac{\partial log x_i}{\partial log p_j}$

- **Engel Aggregation**: total expenditure must change by an amount equal to any wealth change: $\sum_{i=1}^n s_i \eta_i = 1$.

- **Cournot Aggregation**: total expenditure cannot change in response to a change in price: $\sum_{i=1}^n s_i \epsilon_{ij} = -s_j$. 

- v(p,w): **indirect utility function** is the value of the CP: the **welfare of a consumer facing prices p with income w**. Properties:

    a) Homogeneity of degree 0: v($\alpha p, \alpha w$) = v(p,w).
    
    b) Strictly increasing in w and non-increasing in $p_i \forall i$.
    
    c) Quasi-convex: {(p,w)|v(p,w) ≤ v} is convex for any v.
    
    d) Continuous in p and w.
    
- **Theorem of the Maximum(Berge's)**: provides conditions for the continuity of an optimized function and the set of its maximizers with respect to its parameters. Formal definition: given A $\subset$ $\mathbb{R}^N$ and the closed set Y $\subset$ $\mathbb{R}^K$, the correspondence f: A $\rightrightarrows$ Y is *upper hemicontinuous* if it has a closed graph and images of compact sets are bounded.

- **Upper/Lower Hemicontinuity**: the notion of continuous is not immediately extensible to multivalued mappings or correspondences between two sets A and B. a function is upper hemicontinuous when (1) a convergent sequence of points in the domain maps to a sequence of sets in the range which (2) contain another convergent sequence, then the image of limiting point in the domain must contain the limit of the sequence in the range. Lower hemicontinuity essentially reverses this, saying if a sequence in the domain converges, given a point in the range of the limit, then you can find a sub-sequence whose image contains a convergent sequence to the given point. 

- **Closed graph**: for any two sequences $x^m \rightarrow x$ and $y^m \rightarrow y$ with $x^m \in A$ and $y^m \in f(x^m)$ $\forall$ m, then $y \in f(x)$. 

- Images of compact sets are bounded: $\forall$ compact set B $\subset$ A, the set f(B) = {$y \in Y: y \in f(x)$ for some $x \in B$} is bounded.

- **Expenditure Minimization Problem**: it is the minimal level of wealth required to achieve utility $u_0$: $min_{x \in \mathbb{R}^{L+}}$px s.t. u(x) ≥ $u_0$. 

- **Duality**: relation between UMP and EMP: suppose u(x) is a continuous utility function satisfying local nonsatiation and p $\gg$ 0. Then: a) if $x^\ast$ is a solution to the UMP with wealth w, then $x^\ast$ is a solution to the EMP with utility u($x^\ast$); b) if $x^\ast$ is a solution to the EMP with utility $u_0$, then $x^\ast$ is a solution to the UMP with wealth $px^\ast$. 

Given the EMP, we define two functions:

- e(p,u): **Expenditure function** = $min_{x \in \mathbb{R}^{L+}, s.t. u(x) ≥ u}$ px or $min_{x \in C_u}px$. 

- h(p,u): **Hicksian demand correspondence**: solves EMP at price *p* and utility *u*. h(p,u) $\equiv$ {$z \in \mathbb{R}^{L+}|$ pz = e(p,u) and u(z) ≥ u} or h(p,u) $\equiv$ argmin$_{z \in C_u}$ pz. It is used to answer: how would demand change if we change prices from p to p' **and** give whatever compensation is necessary so the utility level is unchanged. Properties:

    a) homogeneous of degree 0 in p, i.e. h($\alpha p,u$) = h(p,u), $\forall \alpha p,u$.
    
    b) No excess utility: u(x) = u $\forall x \in h(p,u)$.
    
    c) If u is strictly quasi-concave, then each h(p,u) contains a single element, i.e. h(p,u) is a function. 

- **Compensated Demand**: another name for the HDC given that: h(p,u) = x(p,e(p,u)). 

- FOC HCD: $p_i = \lambda \frac{du}{dx_i}(h(p,u))$ if $h_i(p,u)$ > 0; $p_i ≥ \lambda \frac{du}{dx_i}(h(p,u))$ if $h_i$(p,u) = 0.

- **Envelope Theorem**: shows that, in a certain sense, changes in the optimizer of the objective do not contribute to the change in the objective function. Important for comparative statics and optimization models. 

- **Slutsky Substitution**: given change in prices, the consumer is given enough money to purchase her old bundle at the new prices, and her choice changes are seen. We can use Slutsky to go from Marshallian to derivatives of the Hicksian. The **Slutsky substitution matrix** S(p,w) is the L x L matrix with $S_{ij} = \frac{\partial x_i}{\partial p_j} + \frac{\partial x_i}{\partial w}x_j$. This means that given the change in $x_i$ resulting from the change in $p_j$ and giving consumer wealth $x_j dp_j$ to make the old bundle affordable, the rise of price of the good i influences the consumption of good j: 1) change in relative price of the k goods(substitution effect); and 2) reduction of real income of consumer(income effect). The Slutsky equation allows the desired decomposition of price comparative statics into income and substitution effects: $\underbrace{\frac{\partial x_i (p,w)}{\partial p_j}}_\text{total effect} = \underbrace{\frac{\partial h_i (p,u)}{\partial p_j}}_\text{substitution effect} - \underbrace{\frac{\partial x_i (p,w)}{\partial p_w}x_j(p,w)}_\text{income effect}$. So the intuition is: **when the price of good j increases**, there are two effects on demand for good *i*: 1) substitution effect captured by the movement on the consumer's original indifference curve; 2) income effect moving from one indifference curve to the other. We have to take into account:

- **Regular good**: $x_i(p,w)$ is decreasing in $p_i$. It is a **Giffen good** if $x_i(p,w)$ is increasing in $p_i$.

- **Substitute good**: *i* substitutes for *j* if $h_i$(p,u)(Hicksian demand correspondence) is increasing $p_j$. It is a complement if $h_i (p,u)$ is decreasing in $p_j$.

- **Gross substitute**: *i* is a gross substitute for *j* if $x_i$(p,w)(Walrasian Demand Correspondence) is increasing in $p_j$. It is a gross complement if $x_i$(p,w) is decreasing in $p_j$.

- **Roy's Identity**: the lemma relates the ordinary (Marshallian) demand function to the derivatives of the indirect utility function. Specifically, denoting the indirect utility function as v(p,w), the Marshallian demand function for good i can be calculated as: $x_i^m$ = $\frac{\frac{\partial v}{\partial p_i}}{\frac{\partial v}{\partial w}}$ where p is the price vector of goods and w is income. Proposition: 

Suppose that u($\circ$) is continuous utility function and preferences are locally nonsatiated and strictly convex. Suppose that the indirect utility function is differentiable at ($p^{\ast},w^{\ast}$) $\gg$ 0. Then for every i = 1, ..., L,

<center>$x_i(p^{\ast},w^{\ast})$ = -$\frac{\partial v (p^{\ast},w^{\ast})}{\partial p_i}$ / $\frac{\partial v (p^{\ast},w^{\ast})}{\partial w}$ </center>

### Putting Consumer Theory to Work

- **Equivalent Variation**; how much money would compensate the change in utility? This is formally denoted by: $EV(p_0, p_1, w) = e(p^0,u_1) - e(p^0, u_0) = e(p^0,u_1) - w$.

- **Compensating Variation(CV)**: given that the prices have changed, how much the consumer would need to be paid to be as well off? This is formally denoted by: $CV(p_0,p_1,w) = e(p^1, u_1) - e(p^1,u_0) = w - e(p^1,u_0)$. 

- **Laspeyres Price Index**: overstates inflation: $\frac{p^1 x^0}{p^0 x^0}$.

- **Paasche Price Index**: understates inflation: $\frac{p^1 x^1}{p^0 x^1}$. 

- **Fisher Ideal Index**: $\sqrt{\text{Paasche x Laspeyres}}$. 

- **Restrict preferences**: simpler function, takes into account only what we think is relevant for our analysis.

- **Restrict price movements**: considering only the $\Delta$prices in the goods we are evaluating.

- **Integrability**: ensures x corresponds to a particular u. A set of continuously differentiable functions $x_i$ are the demand functions generated by some increasing, quasi-concave utility function u if they satisfy Walras' Law and have a symmetric semidefinite Slutsky substitution matrix.

- **Homothetic preferences**: each indifference curve is a magnified/reduced version of the other: $\forall x,y,\alpha >0$: $x \succeq y \implies \alpha x \succeq \alpha y$. This means demand is homogeneous of degree 1 in income. 

- **Gorman(quasi-homothetic)**: the class of preferences in which aggregate demand depends only on aggregate wealth. Defined by: v(p,w) = a(p) + b(p)w. This happens when aggregate demand is not affected by any redistribution of wealth $\implies$ wealth effects are the same accross individuals and accross wealth levels. An indirect utility function is $v(p,w) = (w-a(p))/b(p)$. Parag says: Gorman is a special case where we can do aggregation independently of the wealth distribution and maximize V(p,w) = $\sum_k a^k(p) + b(p)w$. This means the representative consumer's utility is the utilitarian social welfare function W($u^1,...,u^K$) = $\sum u^k$. 

- **Quasi-linearity**: class of preferences for which aggregate demand depends only on aggregate wealth is quasilinear preferences, assuming that some income is spent on the **numeraire good**. With these preferences, the indirect utility function takes the form v(p,w) = a(p) + w. 

- **Demand aggregation**: the demand of the whole market, derived from a "representative consumer". Two classic cases: 1) identical homothetic preferences: $x^k(p,w^k) = w^k \beta(p)$; 2) if individual preferences are homothetic but not identical and incomes are proportional $\implies \exists$ single preference ordering that generates aggregate demand. The question now is: *can we write aggregate demand as equal to the total wealth*? Depends! Only if the wealth effects are the same accross individuals and accross wealth levels. **Proposition**: demand is a function of aggregate wealth $\iff$ the preferences are represented by indirect utility functions of the form: $v^k (p,w^k) = a^k(p) + b(p)w^k$. 

### Producer Theory and Monopoly

- **Producer Theory** is very similar to the Consumer Theory: now firms want to maximize profits while minimizing costs.

- **Profit maximization problem**: $\pi(p) = max_{y \in Y}y p$. 

- c(p,q): **the cost minimization problem**: c(p,q) $\equiv min_{z \in \mathbb{R}^{L+1}} pz$ such that $f(z_1,...,z_{L-1})= q$. 

- **FOC Approach to Comparative Statics**: economists usually are concerned about comparative statics: ex: if we change wealth, what's the effect in consumption? 

- **Increasing Differences**: f(x,$\theta$) satisfies **increasing differences** if $\forall x', x''$ with x' < x'', f(x'',$\theta$) - f(x',$\theta$) is weakly increasing in $\theta$. This means that the incremental gain from choosing x'' over x' (choosing the higher x) is higher when $\theta$ is higher.

- **Topkis Monotonicity Theorem**: let $x^\ast(\theta)$ be as defined in the slides. If f(x,$\theta$) has ID, then $x^\ast (\theta)$ is weakly increasing in $\theta$. 

- **Cobb-Douglas**: f(k,l) = $k^\alpha l^\beta$. Derive: $\frac{\partial^2 f}{\partial k \partial l} = \alpha \beta k^{\alpha - 1}l^{\beta -1} > 0$. 

- **Cross-Price effects**: a) if *k* and *l* are *complements*, then k$^\ast$(r,w) is weakly decreasing in w and $l^\ast(r,w)$ is weakly decreasing in r(rental rate of capital); b) if *k* and *l* are *substitutes*, then $k^ast(r,w)$ is weakly increasing in w and $l^ast(r,w)$ is weakly increasing in r.

- **Single crossing**: weaker condition we can use to characterize a function: a function is SC if $\forall x'>x$ and $\theta' > \theta$: i) f(x',$\theta$) - f(x,$\theta$) ≥ 0 $\implies$ that f(x',$\theta$') - f(x,$\theta$') ≥ 0; ii) f(x',$\theta$) - f(x,$\theta$) > 0 $\implies$ that f(x',$\theta$') - f(x,$\theta$') > 0.

- **Monotonic Selection Theorem(Milgrom-Shannon)**: if f is strict single-crossing in (x,$\theta$), then $x^\ast$($\theta, \overline{X}$) is weakly increasing in $\theta \forall \overline{X} \subset X$. Moreover, if $x^\ast$($\theta,\overline{X}$) is non-decreasing in $\theta$ for all finite sets $\overline{X} \subset \mathbb{R}$, then f is single-crossing in (x,$\theta$). 

- **Multivariate Comparative Statistics**: dealing with more than one variable: $max_{(x_1,x_2)\in X\subset \mathbb{R}^2}F(x_1,x_2,t)$. If F has ID in $(x_1,t)$, then the optimal value of $x_1$ holding $x_2$ constant is non-decreasing in t. The same counts for $x_2$ holding $x_1$ constant. 

- **Supermodularity**: given x,y $\in \mathbb{R}^n$, define operations **meet** and **join** as follows: x $\wedge$ y = $(min,\{x_1,y_1\},...,min\{x_1,y_1\})$ and $x \vee y$ = $ (max,\{x_1,y_1\},...,max\{x_1,y_1\})$. 

- **Sublattice**: a set X $\subset \mathbb{R}^n$ if $\forall x,y \in X$ we have $x \wedge y \ in X$ and x $\vee$ y $\in X$. 

- **LeChatelier Principle**: a firm reacts more input prices in the long-run than in the short-run because it has more inputs it can adjust. For ex: an increase in wage implies an increased use in capital in the long run. Since in the short run the increase in wage makes the firm use less labor, the marginal productivity of labor will decrease and in the long run the firm will use less labor. If inputs are complements, the lesser use of wage will lead to reduced use of capital in the long run. 

- **Monopoly Pricing**: the profit-maximization problem in producer theory is usually unrealistic in that *p* is taken to be independent of *q*. The monopolist os more wary of the perverse effect of high price on demand when consumers react to a price increase by greatly reducing their demand. 

### **Externalities**

- **Externalities** are present when market failures arise. 

- **Pareto Optimal allocation**: there is no other allocation that can make one consumer better off without hurting another. 

- In order to rebalance the welfare given the harm produced by the externality, the government can intervene: **quotas** or **taxes**. 

- **Coase Theorem**: externality problems arise because: 1) lack of clearly specified property rights; 2) transaction costs. He states *if property rights are clearly specified and there are no transaction costs, bargaining will lead to an efficient outcome no matter how the rights are allocated*. 

- **Anti-Coase Theorem**: *no bargaining procedure can lead to an efficient outcome for all values of b and s when they are private information and independently distributed*. So, bargaining won't lead to equilibrium because information asymmetries exist. 

- **Missing Markets**: if we can define the property rights on an externality, why isn't there a market for it? 

- **Price vs Quantity**: how should the government intervene: taxing prices or quantities? 

- **Optimal Number**: the optimal number of equilibrium is the one that **maximizes aggregate surplus**. 

_ **Equilbrium Number**: demand equals supply and there are no profits(free-access). 

### Introduction to General Equilibrium

- **Walrasian Equilibrium**: need 3 things: 1) agents maximize utilities; 2) markets clear; 3) firms choose production to maximize profit. For now, everyone takes prices as given. We evaluate *relative* prices, not absolute ones.

- We start from a 2x2 exchange economy, no production. 2 consumers(i) and 2 goods($\ell$). Consumption vector is $x^i = (x^i_1,x^i_2)$. Each agent is initially endowed with $\omega^i = (w_1^i, \omega_2^i) \in \mathbb{R}^2_+$ where $\omega_\ell^i$ is $i$ endowment of commodity $\ell$. Aggregate endowment is $\omega = \omega_1 + \omega_2$. Agg endowment for good $\ell$ is $\omega_\ell = \omega_ell^1 + \omega_\ell^2$. The **exchange economy** is then: $\mathcal{E}= (u_i,\omega^i)_{i=1,2}$. 

- **Edgeworth Box**: the best visual tool we have to analyze this 2x2 economy. 

- **Budget Set**: may extend to outside the Edgeworth box.

- **Offer curve**: vary the price $\rightarrow$ the budget constraint changes. At each point on offer curve, budget constraint is tangent to indifference curve.

- **Excess demand**: total agent demand for a commodity minus what is available in the economy.

- **Pareto Set**: allocations where indifference curves are tangent. If utilities are differentiable, Pareto allocations are characterized by: $\frac{\partial u^1}{\partial x_1} / \frac{\partial u^1}{\partial x_2}(x^1) = \frac{\partial u^2}{\partial x_1} / \frac{\partial u^2}{\partial x_2} (\omega - x^1)$.

- **Contract curve**: part of Pareto set where consumers do at least as well as initial endowment.

- **First Welfare Theorem**: If utility is increasing, then allocation ($x^i)_{i \in \mathcal{I}}$ is Pareto optimal. Support for Adam Smith's invisible hand. If $\omega$ and *p* are given and each agent maximizes utility, at the solution the consumption bundles $x^1,...,x^l$, the UMP KT conditions hold and each consumer's budget constraint is satisfied. 

- **Second Welfare Theorem**: If ($\omega^i)_{i \in \mathcal{I}}$ is Pareto optimal, then there exists a price vector $p \in \mathbb{R}_+^{L}$ such that (p,($\omega^i)_{i \in \mathcal{I}})$ is a Walrasian equilibrium in $\mathcal{E}$. In other words, once we find an efficient allocation of wealth, we could support it with a price equilibrium. Also been used to say that any redistributional goals can be achieved from lump-sum reallocations of endowments and letting the market play out. If the KT conditions for the Pareto program are satisfied, then define prices $p_\ell = \mu_\ell$ and Lagrange multipliers $\gamma^i = \frac{1}{\lambda^i}$. 

- **Separating Hyperplane Theorem**: there exists a price vector that separates the convex and non-empty subsets of $\mathbb{R}^L$ B,C such that none element of B is in C and vice-versa. Formally: sup p B ≤ inf p C.

- **Supporting Hyperplane Theorem**: there exists a price vector such that touches the limit of the convex set B. 

- **Robinson Crusoe Model**: we now incorporate production while considering only one consumer and one firm(owned by that consumer). We incorporate some useful insights before we advance to the more realistic model:

- **2x2 Production Economy**: 2 goods and 2 factors. We work with per unit cost and factor demands. I find the most useful way to get insights to this is to do the exercises.

- **Public Goods**: a good for which the use of a unit by one consumer does not preclude its use by others. Can be non-excludable(lighthouse) and excludable(pay-per-view TV). Its market provision depends on what the consumers think that the other consumers will demand, which leads to a strategic problem. In a competitive equilibrium there is under-provision of the public good relative to the level prescribed by the Samuelson condition. **Free-riding** arises. 

- **Personalized prices**: Lindahl suggested that the amount purchsaed by each agent should be accountable so that each agent face a **personalized price** $p^i$. The proposition is that in this Lindahl equilibrium the market would be in a Pareto optimal. However, this is a unrealistic premise. Market mechanisms are unlikely to provide public goods efficiently $\rightarrow$ government should get involved. 

### GE Existence and Other Properties

For Walrasian Equilibrium, let's consider:

- **Brouwer's Theorem**: existence of Walrasian Equilibrium. Suppose A $\subset \mathbb{R}^L$ is nonempty, convex, compact and f: $A \rightarrow A$ is continuous. Then *f* has a fixed point: $\exists x^\ast \in A$ such that f($x^\ast$) = $x^\ast$. 

- **Kakutani's Fixed Point Theorem**: suppose A $\subset \mathbb{R}^L$ is nonempty, compact, convex and f: A $\rightarrow$ A is a correspondence such that: i) f is non-empty; ii) f is convex valued; iii) f is upper hemi-continuous. Thne f has a fixed point: $\exists x^\ast \in A$ such that $x^\ast \in f(x^\ast)$. 

- In a Walrasian model what matters are the relative prices: so we can normalize to simples. We focus on a pure exchange economy with our four assumptions + strong monotonicity. **excess demand of consumer** i: $z^i (p) = x^i(p,p \omega^i) - \omega^i$; **aggregate excess demand**: z(p) = $\sum_i z^i(p)$. 

- z(p): aggregate excess demand. Properties:

    a) z is a continuous function.
    
    b) homogeneity of degree 0. 
    
    c) walras' law: pz(p) = 0 $\forall p$.
    
    d) bounded below: z(p) ≥ - $\overline{\omega}$.
    
    e) boundary condition: $p^n \rightarrow p$ when p ≠ 0 but $p_\ell = 0$ for some $\ell$, then max{$z_1(p^n),...,z_L(p^n)$} $\rightarrow \infty$. 
    
- **Jacobian Matrix**: represents the first-order partial derivatives of a vector-valued function with several variables. 

- **Properties of Walrasian equilibrium**:

    a) any regular equilibrium is locally unique.
    
    b) a regular economy has a finite number of equilibria.
    
    c) for almost every vector of initial endowments, the exchange economy is regular.
    
- **Sonnenschein-Mantel-Debreu Theorem**: anything goes: any potential excess demand function can be in fact a demand function. Now we're not restricted by the rationality restrictions of individual demand that were equivalent to Slutsky symmetry and negative semi-definiteness.

### Microfoundations for GE

- **The core**: is the set of all allocations which cannot be improved on by any coalition. The set of all allocations such that no coalition(set of agents) can improve on or block the allocation by seceding from the economy and trade only among its members. The central result is: "core convergence" $\implies$ for economies with a large number of agents(or with agents with low power), core allocations are "approximately Walrasian". Every core allocation in an exchange economy is Pareto optimal. Every Walrasian equilibrium lies in the core. 

- **Coalition**: is a set S $\subset$ {1,...,I}. A coalition S blocks or improves on allocation x by $\hat{x}$ if $\sum_{i \in S} \hat{x}^i = \sum_{i \in S}\omega^i$ and for all agents i, $\hat{x}^i \succeq_i x^i$ with at least one preference strict. 

##### **Core Convergence**

- The set of types of consumers is $\mathcal{I} = ${1,...,I}. 

- R-replica economy $\mathcal{E}_R$ is one with R consumers of each type.

- Total number of consumers is RI. 

- **Equal Treatment Proposition**: assume consumers have strictly convex preferences. If x is a core allocation in $\mathcal{E}_R$, then $x^i,r'$ $\forall$ r,r' $\rightarrow$ consumers of the same type consume the same bundle. 

- **Debreu Scarf Theorem**: $\cap_R C_R = W$, that is: if $x^\ast \in C_R \forall R$, then $x^\ast$ is Walrasian. 

### GE with indivisibilities

- Some simple models with indivisible goods(housing market).

- **Individually Rational matching**: every agent receives a house at least as good as her initial house. 

- **Theorem**: there is a competitive equilibrium matching for a housing market which is given by the core matching. 
...
- [**Shepard's Lemma**](https://en.wikipedia.org/wiki/Shephard%27s_lemma):  if indifference curves of the expenditure or cost function are convex, then the cost minimizing point of a given good $i$ with price $p_i$ is unique. The idea is that a consumer will buy a unique ideal amount of each item to minimize the price for obtaining a certain level of utility given the price of goods in the market.

- [**Cobb-Douglas utility function**](https://en.wikipedia.org/wiki/Cobb%E2%80%93Douglas_production_function): is a particular form of the **production function**, used to represent the relationship between two or more inputs(usually capital and labor). The standard form: Y = AL$^\beta$K$^\alpha$, where: **Y** = total production; **L** = labor input; **K** = capital input; **A** = total factor productivity; $\alpha$ and $\beta$ are the output elasticities of capital and labor, respectively. These values are constants determined by available technology. In terms of return to scale: if $\alpha + \beta$ > 1, increasing returns to scale; = 1, constant returns to scale; < 1 decreasing returns to scale. 

- Jensen's Inequality: generalizes the statement that the secant line of a convex function lies above the graph of the function, which is in Jensen's Inequality for two points: the secant line consists of weighted means of the convex function for t $\in$ [0,1]. Represented by: $af(x_1) + (1 - a)f(x_2) ≥ f(ax_1 + (1 - a)x_2)$.

- [**Theorem of the maximum**](https://en.wikipedia.org/wiki/Maximum_theorem): if the elements of an optimization problem are sufficiently continuous, then some but not all of that continuity is preserved in the solutions. Let's organize the setting: i) **parameter space**: $\Theta$; ii) **function to be maximized**: $f(x,\theta)$; iii) **constraint set that f(x|$\theta$) will be maximized over**: C($\theta$). $\rightarrow$ $f^\ast(\theta)$ is the maximized value of the function and $C^\ast$ is the set of points that maximize $f$. 

    - Formally: let X and $\Theta$ be topological spaces, $f: X \times \Theta \rightarrow \mathbb{R}$ be a continuous function on the product X $\times \Theta$, and C : $\Theta \rightrightarrows X$ be a compact-valued correspondence such that C($\theta$) = $\varnothing \forall \theta \in \Theta$. The marginal/value function $f^\ast: \Theta \rightarrow \mathbb{R}$ by: $f^\ast (\theta) = sup \{f(x,\theta): x \in C(\theta)\}$ and the set of maximizers $C^\ast: \Theta \rightrightarrows X$ by $C^\ast (\theta) = arg sup \{f (x,\theta): x \in C(\theta)\}$ = {$x \in C(\theta): f(x,\theta) = f^\ast (\theta)$}. If C is continuous (both upper and lower hemicontinuous) at $\theta$, then $f^\ast$ is continuous and $C^\ast$ is upper hemicontinuous with nonempty and compact values. As a consequence, the sup may be replaced by max and the arg sup by arg max. 

- [**Hemicontinuous function**](https://en.wikipedia.org/wiki/Hemicontinuity): In mathematics, the notion of the continuity of functions is not immediately extensible to multivalued mappings or **correspondences** between two sets A and B. The dual concepts of upper hemicontinuity and lower hemicontinuity facilitate such an extension. A correspondence that has both properties is said to be continuous in an analogy to the property of the same name for functions. **Upper continuity** implies that when a convergent sequence of points in the domain maps to a sequence of sets in the range which contain another convergent sequence, then the image of limiting point in the domain must contain the limit of the sequence in the range. **Lower hemicontinuity** reverses this: if a sequence in the domain converges, given a point in the range of the limit, then you can find a sub-sequence whose image contains a convergent sequence to the given point. 

- [**Gorman**](https://en.wikipedia.org/wiki/Gorman_polar_form): the class of preferences in which aggregate demand depends only on aggregate wealth.

- FWT Proof:

- [**Separating Hyperplanes**](https://en.wikipedia.org/wiki/Hyperplane_separation_theorem): theorem about disjoint sets that comes into play when we're talking about allocation efficiency. If B,C are convex and non-empty subsets of $\mathbb{R}^L$ and don't share any elements $\implies \exists$ p $\in \mathbb{R}^L$ such that *sup pB ≤ inf pC*. The **Supporting Hyperplane** states something similar: if B $\subseteq$ $\mathbb{R}^L$ is convex and that x is not an element of the interior of the set B $\implies \exists$ p $\in \mathbb{R}^L$ with p ≠ 0 such that px ≥ py $\forall y \in$ B. 
