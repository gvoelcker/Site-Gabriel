...
Suppose that potential outcomes can be written:

\[ Y_{si} = f_i(s) \]

\[ f_i(s) = \alpha + \rho s + \eta_i \]

Suppose also: $\eta_i = A_i' \gamma + v_i$, where $A_i$ is the ability we want to take into account in a regression of wages to schooling. A long regression would then be written:

\[ Y_i = \alpha + \rho s_i + A_i' \gamma + v_i \]

How do estimate the long regression coefficient $\rho$ when $A_i$ is unobserved? We need a **instrument**! It has to be correlated to the causal variable of interest($s_i$) but not correlated to any of the other determinants of the dependent variable(Cov($\eta_i, z_i$)=0). This uncorrelated part is called the **exclusion restriction**. Given the exclusion restriction, we have that:

$\beta_{IV} = \rho = \frac{Cov(Y_i,Z_i)}{Cov(S_i,Z_i)}$

The coefficient of interest is the ratio of the population regression of $Y_i$ on $Z_i$ (the reduced form) to the population regression of $s_i$ on $z_i$ (the first stage). This first stage is the regression of the causal variable of interest on covariates and instruments. Once we have that result, we estimate our relation of interest. For example:

\[ \text{First stage: } s_i = X_i' \pi_{10} + \pi_{11}z_i + \xi_{1i} \]

\[ \text{Reduced form: } Y_i = X_i' \pi_{20} + \pi_{21}z_i + \xi_{2i} \]

The parameter $\pi_{11}$ captures the first-stage effect $Z_i$ on $S_i$, adjusting for covariates, $X_i$. The parameter $\pi_{21}$ captures the reduced-form effect of $Z_i$ on $Y_i$, adjusting for these same covariates. The dependent variables in these two equations are said to be **endogenous variables** (determined jointly within the system) and the variables on the right-hand side are said to be the **exogenous variables**. The covariate-adjusted IV estimator is the sample analog of $\frac{\pi_{21}}{\pi_{11}}$. 
...
What is the **link** between 2SLS and IV? We might want to combine alternative IV estimates into a single more precise estimate. In models with multiple instruments, 2SLS accomplishes this by combining multiple instruments into a single instrument. We can also add interaction terms to the instrument list, which increases the first-stage $R^2$, that goes up because the quarter-of-birth pattern in schooling differs across cohorts. 
...
- **Exogenous variable**: includes the *exogenous covariates not instrumented* and the *instruments themselves*. In a simultaneous equations model, exogenous ariables are determined outside the system. We think of them as **controls**. Ex: Angrist&Krueger(1991) use dummies for year of birth and state of birth. 

##### **Wald Estimator**

Simplest IV estimator: uses a single dummy instrument to estimate a model with one endogenous regressor and no covariates. A Wald estimator caputres the causal effect of the treatment ($D_i$) because the only reason why $E[Y_i|Z_i]$ changes is the variation in $E[D_i|Z_i]$. 
...
The GMM interpretation of 2SLS highlights the fact that IV estimates can be constructed from sample moments alone, with no microdata. 

- **Two-sample instrumental variables**: if the data sets come from the same population, we can use two-sample instrumental variables. Assuming that the plim of both is equal, GLS estimates of the two-sample moment equation:

\[ \frac{Z_1'Y_1}{N_1} = \frac{Z_2'W_2}{N_2} \Gamma + \{ [ \frac{Z_1'W_1}{N_1} - \frac{Z_2'W_2}{N_2}] \Gamma + \frac{Z_1'\eta_1}{N_1} \} \]

Are consistent for the coefficient vector $\Gamma$ ($[\alpha', \rho]'$). This widens the scope for IV estimation: we can use different samples! Two further simplifications: when the instruments consist of a full set of mutually exclusive dummy variables, the second moment equation simplifies to a model for conditional means. The 2SLS minimand for the two-sample problem becomes:

\[ \hat{J}_N (\hat{g}) = \sum_j \omega_j (\hat{y}_{1j} - \hat{g}' \overline{W}_{2j})^2\]

Where $\overline{y}_{1j}$ is the mean of the depend variable at instrument/covariate value *j* in one sample, $\overline{W}_{2j}$ is the mean of endogenous variables and covariates at instrument/covariate value $j$ in a second sample, and $\omega_j$ is an appropriate weight. This amounts to WLS estimation of the VIV equation, except that the dependent variable and independent variables do not come from the same sample. 

#### **4.4. IV with Heterogeneous Potential Outcomes**

So far, we've considered a constant causal effect for IV: $Y_{1i} - Y_{0i} = \rho \forall i$. Why heterogeneity in treatment effects important? In terms of **internal validity** and **external validity**, we have to understand whether the result of a good IV study can be applied to different contexts. An econometric framework with heterogeneous treatment effects helps us to assess both the internal and external validity of IV estimates. 

##### **4.4.1 Local Average Treatment Effects**

We can think of instrumental variables as initiating a causal chain where the instruments $Z_i$ affects the variable of interest $D_i$, which in turn affects the outcomes $Y_i$. In a heterogeneous effects model, we start by assuming that the instrument is as good as randomly assigned. This is sufficient for a causal interpretation of the reduced form. The second key assumption is that $Y_i(d,z)$ is a function only of *d* (exclusion restriction). Using the exclusion restriction, we can define potential outcomes indexed solely agains treatment status using the single index notation ($Y_{0i}, Y_{1i}$). The observed outcome can thus be written in terms of potential outcomes as: 

\[ Y_{0i} + (Y_{1i} - Y_{0i})D_i \]

The last assumption is **monotonicity**: the instrument can only affect the observations in the same way: either $D_{1i} ≥ D_{0i}$ or $D_{1i} ≤ D_{0i}$. 

Given all the assumptions (exlusion restriction, independence of instruments and potential outcomes, existence of a 1st-stage and monotonicity), the Wald estimand can be interpreted as the Local Average Treatment Effect:  
\[ \frac{E[Y_i|Z_i = 1] - E[Y_i|Z_i = 0]}{E[D_i|Z_i = 1] - E[D_i|Z_i = 0]} = E[Y_{1i} - Y_{0i}|D_{1i} > D_{0i}] = E[\rho_1|\pi_{1i > 0}] \]

The LATE theorem states that an instrument that is as good as randomly assigned, affects the outcome through a single known channel, has a 1st stage and affects the causal channel of interest only in one direction can be used to **estimate the average causal effect on the affected group**. 

##### **4.4.2** The Compliant Subpopulation

LATE partitions any population into a set of three instrument-dependent subgroups: 

*i) compliers*: the subpopulation with $D_{1i} = 1$ and $D_{0i} = 0$.

*ii) always-takers*: the subpopulation with $D_{1i} = D_{0i} = 1$.

*iii) never-takers*: the subpopulation with $D_{1i} = D_{0i} = 0$.

So we can say that the treated are composed of either always-takes or of compliers with the instrument switched on. So we get:

\[ \underbrace{E[Y_{1i} - Y_{0i}| D_i = 1]}_\text{Effect on the treated} + \]

\[ = \underbrace{E[Y_{1i} - Y_{0i}| D_{0i} = 1]}_\text{Effect on always-takers}P[D_{0i} = 1|D_i = 1] + \underbrace{E[Y_{1i} - Y_{0i}|D_{1i} > D_{0i}]}_\text{Effect on compliers}P[D_{1i} > D_{0i}, Z_i = 1| D_i = 1] \]

Since $P[D_{0i} = 1|D_i = 1]$ and $P[D{1i} > D_{0i}, Z_i = 1|D_i = 1]$ add up to one, it means that the effect of TOT is a weighted average of effects on always-takers and compliers.

Looking on the other direction, LATE is **not** the average causal effect of treatment on the nontreated: E$[Y_{1i} - Y_{0i}|D_i = 0]$. The average effect of treatment on the nontreated is a weighted average of effects on never-takers and compliers.

\[ \underbrace{E[Y_{1i} - Y_{0i}| D_{i} = 0]}_\text{Effect on the nontreated} \]

\[ \underbrace{E[Y_{1i} - Y_{0i}| D_{1i} = 0]}_\text{Effect on never-takers} P[D_{1i} = 0|D_i = 0] + \underbrace{E[Y_{1i} - Y_{0i}| D_{1i} > D_{0i}]}_\text{Effect on compliers} P[D_{1i} > D_{0i}, Z_i = 0|D_i = 0]\]

If we average the equations fo the effect on the treated and on the nontreated, we get the unconditional average treatment effect to be a weighted average of effects on complies, always-takers and never-takers.

##### **4.4.3 IV in Randomized Trials**

What happens if we compare results considering whether the subjects were offered treatment? We look at **intention-to-treat effects**. They have a causal interpretation: they tell us the causal effect of the offer of treatment, building in the fact that some of those offered treatment declined to participate. ITT divided by the difference in compliance rates between treatment and control groups as originally assigned measure the ffect of treatment on the treated. **We use the fact the we have almost no always-takers in LATE to show the effect of TOT**. This is known as the Bloom result: 

\[ \frac{E[Y_i|Z_i = 1] - E[Y_i|Z_i = 0]}{P[D_i = 1|Z_i = 1]} = E[Y_{1i} - Y_{0i}|D_i = 1] \]

The best example is the Minneapolis Domestic Violence Experiment: those who were assigned to be arrested were in fact arrested! No never-takers!

##### **4.4.4 Counting and Characterizing Compliers**

We've seen that each instrumental variable identifies a unique causal parameter, one specific to the subpopulation of compliers for that instrument. Although different IV estimates are implicitly weighted up by 2SLS to produce a single average causal effect, overidentification testing where multiple instruments are validated regardless whether they are estimating the same thing is out the window in a fully heterogeneous world. Differences in compliant subpopulations might explain variability in treatment effects from one instrument to another. That's why it's important to know as much as we can about the compliers for different instruments. 

On the other hand, **if the compliant subpopulations** associated with two or more instruments **are very different**, then we are prepared to adopt **homogeneous effects** as a working hypothesis. 

#### **4.5. Generalizing LATE**

The LATE theorem applies to a stripped-down causal model in which a single dummy instrument is used to estimate the impact of a dummy treatment with no covariates. We can generalize this in three important ways: 1) multiple instruments; 2) models with covariates; 3) models with variable and continuous treatment intensity. In those three cases, the IV estimand is a weighted average of causal effects for instrument-specific compliers. The econometric tool remains 2SLS and the interpretation remains fundamentally similar to the basic LATE result, with a few bells and whistles. 2SLS with multiple instruments produces a causal effect that averages IV estimands using the instruments one at a time; 2SLS with covariates produces an average of covariate-specific LATEs; 2SLS with variable or continuous treatment intensity produces a weighted average derivative along the length of a possibly nonlinear causal response function. These results provide a simple casual interpretation for 2SLS in most empirically settings.

##### **4.5.1. LATE with Multiple Instruments**

Similar: use two dummies to construct Wald estimators. We can also use a 2SLS instead of Wald estimators. The resulting grouped data estimator is a linear combination of the underlying estimators. In other words, it is a linear combination of the instrument-specific LATEs using the instruments one at a time. So we can say that 2SLS using dummies $Z_{1i}$ and $Z_{2i}$ is a weighted average of IV estimates using $Z_{1i}$ and $Z_{2i}$ one at a time. Let

\[ \rho_j = \frac{Cov(Y_i,Z_{ji})}{Co(D_i,Z_{ji});j = 1,2} \]

Denote the two IV estimands using $Z_{1i}$ and $Z_{2i}$. Through MHE's math we see that 2SLS is a weighted average of causal effects for instrument-specific compliant subpopulations. When the two instruments are used together, the resulting 2SLS estimates are a weighted average of the Wald estimates produced by using the instruments one at a time.

##### **4.5.2 Covariates in the Heterogeneous Effects Model**

The LATE theorem does not involve covariates! That is because if the instrument is randomized, it is likely independent of covariates. But what if the instrument is not randomized? We condition on covariates in order to have valid conditional independence and exclusion restrictions underlying IV estimation. In other words, we think of the IVs as being as good as randomly assigned conditional on covariates $X_i$. A second reason for incorporating covariates is that conditioning on covariates may reduce some of the variability in the dependent variable. This can lead to more precise 2SLS estimates. 

**Saturate and Weight Theorem**: 2SLS with a fully saturated first stage and a saturated model for covariates in the second stage produces a weighted average of covariate-specific LATEs. The weights are proportional to the average conditional variance of the population first-stage fitted value, $E[D_i|X_i,Z_i]$, at each value of $X_i$. The theorem comes from the fact that the first-stage coincides with $E[D_i|X_i,Z_i]$ when 4.5.3 is saturated. The vision of 2SLS providing a MMSE approximation to an underlying causal relation was developed by Abadie. We can find the compliers in the following sense:

\[ \text{Theorem 4.5.2: Abadie Kappa: } \kappa_i = 1 - \frac{D_i(1 - Z_i)}{1 - P(Z_i = 1|X_i)} - \frac{(1 - D_i)Z_i}{P(Z_i = 1|X_i)} \]

We finz that $D_i(1 - Z_i) = 1$ are always-takers because they have $D_{0i}$ and those with $(1 - D_i)Z_i = 1$ are never-takers because they have $D_{1i} = 0$. Hence, the compliers are the left out group.

##### **4.5.3 Average Causal Response with Variable Treatment Intensity**

Imagine that now we have a causal variable that takes values [1,2,3...]. What's the effect of going from 1 to 2, 2 to 3...? We now are answering a causal "what if" question for a multinomial $s_i$. 2SLS provides a computational device that generates the weighted average of unit causal effects, with a weighting function we can estimate and study, so as to learn where the action is coming from with a particular instrument. 

- **Average Causal Response Theorem**: states that the Wald estimator with variable treatment intensity is a weighted average of the **unit causal response** along the length of the potentially nonlinear causal relation described by $f_i(s)$. The **unit causal response**, $E[Y_{si} - Y_{s-1,i}|S_{1i} ≥ s > S_{0i}]$, **is the average difference in potential compliers at point s**. 
...
There are other risks associated with manually computing 2SLS, not only the OLS standard errors issue already discussed.

- **Covariate Ambivalence**: 

- **Forbidden Regressions**: 2SLS to nonlinear models. Imagine the dummy variable of interest in the first stage is regressed by OLS: the CEF is probably nonlinear! Let's try (for example) probit instead.

##### **4.6.2 Peer Effects**

Peer effects: the causal effect of group characteristics on individual outcomes. **Spillovers**: social return to schooling. 

##### **4.6.3 Limited Dependent Variables Reprise**

Most nonlinear LDV models are built around a nonlinear transformation of a linear latent index. Then, we use probit/logit/Tobit. But they may not be worth the work. OLS, however, has a conceptual robustness that structural models often lack. In fact we can think of OLS as a scheme that has the virtue of simplicity automation, and comparability across studies. Nonlinear latent index models are more like GLS: they provide an efficiency gain when taken literally, but require a commitment to functional form and distributional assumptions(not always indicated). 

The arguments in favor of conventional OLS with LDVs apply with equal force to 2SLS and models with endogenous variables. IV methods capture local average treatment effects regardless of whether the dependent variable is binary, non-negative or continuously distributed. We can think of 2SLS as estimating LATE averaged across covariate cells. In models with variable or continuous treatment intensity, 2SLS gives us the average causal response to an average derivative. 2SLS estimates LATe directly: no intermediate step involving the calculation of marginal effects. 

2SLS is not the only way to go: we can build up a causal story by describing the proces generating LDVs in detail. Then we see the math notations for potential treatment and outcomes, and the treatment effects for a probit model.
...
Where $\beta = 1$, $\pi_1 = 0.1$ and $\pi_j = 0$ for j = 2,...,Q. Next, we make a Monte-Carlo simulation for: OLS, just-identified IV (2SLS with q = 1), 2SLS with two instruments and LIML with Q = 2. The results (figure 4.6.1) illustrate the virtual unbiasedness of just-identified IV. 2SLS with one weak and one uninformative instrument is moderately biased toward OLS. The distribution function for LIML with Q = 2 is indistinguishable from that for just-identified IV, even though the LIML estimator also uses an uninformative instrument. If we increase the number of instruments to 20, the bias in 2SLS is now much worse (figure 4.6.2). LIML again  performs well and is centered around $\beta = 1$, with more dispersion than in the Q = 2 case. When the simulation reports results from a model that is truly *unidentified*, LIML has an advantage: in figure 4.6.3 the widely dispersed sampling distribution reflects how the data are uninformative about the parameter of interest. 
...
where $T_i = 1(x_i ≥ x_0)$. The dummy variable $T_i$ indicates the point **where E[$D_i|x_i$] is discontinuous**. Fuzzy RD leads to a 2SLS estimation strategy. Assuming that $g_0(x_i)$ and $g_1(x_i)$ can be described by $\rho$th-order polynomials as we did for $f_0(x_i)$ and $f_1(x_i)$, we have:

\[ E[D_i|x_i] = \gamma_{00} + \gamma_{01} x_i + \gamma_{02}x_i^2 + ... + \gamma_{0p}x_i^\rho + [\pi + \gamma_1^\ast x_i + \gamma_2^\ast x_i^2 + ... + \gamma_p^\ast x_i^\rho] T_i \]

\[ = \gamma_{00} + \gamma_{01}x_i + \gamma_{02}x_i^2 + ... + \gamma_{0p}x_i^\rho + \pi T_i + \gamma_1^\ast x_i T_i + \gamma_2^\ast x_i^2 T_i + ... + \gamma_\rho^\ast x_i^\rho T_i\]

Where the $\gamma^\ast$'s are the coefficients on the polynomial interactions with $T_i$. From here we see that $T_i$ and the interaction terms can be used as instruments for $D_i$. The simplest fuzzy RD estimator uses only $T_i$ as an instrument. The resulting just-identified IV estimator has the virtues of transparency and good finite-sample properties. The first stage in this case is: 
...
\[ E[D_i|x_0 ≤ x_i < x_0 + \Delta] - E[D_i|x_0 - \Delta < x_i < x_0] \approx \pi\]
...
The above is local average treatment effect. As with the nonparametric version of sharp RD, the finite-sample behavior of the sample analog of the equation above is not likely to be very good. Ideally, 2SLS estimates in the discontinuity sample with few controls will be broadly consistent with the more precise estimates constructed using the larger sample.
...
### **1) RCT and Regression Recap**

1.1) **Conditional Expectation Function**: $\mathbb{E}(Y_i|X_i)$. For a certain $X_i$ = x, we write: $\mathbb{E}(Y_i|X_i = x)$. 

- For *continuous* $Y_i$ with *conditional density* $f_y(.|X_i = x)$, the CEF is:

\[ \mathbb{E}(Y_i|X_i = x) = \int t f_y(t| X_i = x)dt \]

- For *discrete* $Y_i$ with *condition pmf* $f_y(.|X_i = x)$, the CEF is:

\[ \mathbb{E}(Y_i|X_i = x) = \sum_t t f_y (t|X_i = x) \]

- **Law of Iterated Expectations**: $\mathbb{E}(Y_i) = \mathbb{E}(\mathbb{E}(Y_i|X_i)) \rightarrow$ an unconditional expectation can be written as the population average of the CEF. 

- **CEF Decomposition Property**: any random variable $Y_i$ can be decomposed into a piece explained by $X_i$(the CEF) and a piece left over which is orthogonal (i.e. uncorrelated) with any function of $X_i$. $\mathcal{Y}_i = \mathbb{E}(Y_i|X_i) + \epsilon_i$, where: **i)** $\epsilon_i$ is mean-independent of $X_i$($\mathbb{E}(e_i|X_i)=0$) **ii)** $\epsilon_i$ is uncorrelated with any function of $X_i$ (and so it uncorrelated with the CEF, which is a function of $X_i$). 

Pf: given $Y_i = \mathbb{E}(Y_i|X_i) + \epsilon_i$, we prove:

**i)** $\mathbb{E}(\epsilon_i |X_i) \iff \mathbb{E}(Y_i - \mathbb{E}(Y_i|X_i) | X_i) \iff \mathbb{E}(Y_i|X_i) - \mathbb{E}(Y_i|X_i) = 0$. Note also that $\mathbb{E}(\epsilon_i) \iff \mathbb{E}(\mathbb{E}(\epsilon_i |X_i)) = 0$

**ii)** Let *h* be a function of $X_i$; WTS this function is uncorrelated to $\epsilon_i$: 

\[ \mathbb{E}(h(X_i)\epsilon_i) = \mathbb{E}(\mathbb{E}(h(X_i)\epsilon)|X_i) \iff \mathbb{E}(h(X_i)\mathbb{E}(\epsilon_i|X_i)) = 0\]

So if the expectations are uncorrelated (=0), we have:

\[ Cov(h(X_i),\epsilon_i) = \mathbb{E}(h(X_i)\epsilon_i) - \mathbb{E}(h(X_i))\mathbb{E}(\epsilon_i) = 0\]

Since $\mathbb{E}(\epsilon_i) = \mathbb{E}(h(X_i)\epsilon_i) = 0$. 

- **CEF Solves The MMSE Prediction Problem**: the CEF is the best predictor of $Y_i$ given $X_i$ in that it solves the Minimum Mean Squared Error (MMSE) prediction problem. 

- **ANOVA**: states that: $V(Y_i) = V(\mathbb{E}(Y_i|X_i)) + \mathbb{E}(V(Y_i|X_i))$

Pf: 1) use the CEF Decomposition Property; 2) decompose variance of $\epsilon_i$ and iterate expectations; 3) use the fact that $\epsilon_i = Y_i - \mathbb{E}(Y_i|X_i))$ and find $\mathbb{E}(V(Y_i|X_i))$; 4) Once we know that V($\epsilon_i$) = $\mathbb{E}(\mathbb{E}(\epsilon_i^2|X_i)) = \mathbb{E}(V(Y_i|X_i))$, we have: 5) V($Y_i$) = V($\mathbb{E}(Y_i|X_i)) + \mathbb{E}(V(Y_i|X_i))$.

1.2) **Linear Regression and the CEF**: 

What is the regressor that minimizes the expected $(Y_i - X_i' \beta)^2$? FOC for *b*: $\beta = \mathbb{E}(X_iX_i')^{-1} \mathbb{E}(X_i Y_i)$. The **population residual** is: $e_i = Y_i - X_i'\beta$. 

The beta for a **bivariate regression** is: $\beta_1 = \frac{Cov(Y_i,x_i)}{V(x_i)}$. 

In a **multivariate regression** the slope coefficient for the *k*-th regressor is given by $\beta_k = \frac{Cov(Y_i,\tilde{x}_{ki})}{V(\tilde{x}_{ki})}$. Each coefficient in a multivariate regression is the bivariate slope coefficient for the corresponding regressor, after partialling out all the other variables in the model. 

- **1.3 Regression Justification**: what if the CEF is linear? Then the population regression function is the CEF. Suppose: $\mathbb{E}(Y_i|X_i) = X_i'\beta^\ast$. We know that $\mathbb{E}(X_i(Y_i - \mathbb{E}(Y_i|X_i))) = 0$. So by decomposition property we arrive at $\beta^\ast = \beta$. 

But when is the CEF linear? **i)** *Joint Normality*: vector ($Y_i,X_i$) has a multivariate normal distribution; **ii)** *regression model saturated*: the saturated regression model has a separate parameter for every possible combination of values that the set of regressors can take on.

- **Best Linear Predictor Theorem**: the function $X_i'\beta$, where $\beta = \mathbb{E}(X_iX_i')^{-1} \mathbb{E}(X_iY_i)$ is the best linear predictor of $Y_i$ given $X_i$ in a MMSE sense. Intuition: the CEF $\mathbb{E}(Y_i|X_i)$ is the best MMSE predictor of $Y_i$ given $X_i$ in the class of all functions of $_i$. The population regression function is the best we can do in the class of linear functions.

- **Regression-CEF Theorem**: even if the CEF is nonlinear, the function $X_i'\beta$ provides the MMSE best linear approximation to it. 

- **1.4 Fits and Residuals**: suppose $\alpha$ and $\beta_1,...,\beta_k$ are the intercept and slope coefficients from a regression of $Y_i$ on $X_{1i},..., X_{ki}$. The **fitted values** from this regression are: $\hat{Y}_i = \alpha + \sum_{k=1}^K \beta_k X_{ki}$. The **residuals** are: $e_i = Y_i - \hat{Y}_i = Y_i - \alpha - \sum_{k=1}^K \beta_k X_{ki}$. Properties of the regression residuals: **i)** expectation and mean 0: $\mathbb{E}(e_i) = \sum_{i=1}^n e_i = 0$; **ii)** they are uncorrelated (in population and sample) with all regressors that made them and with the corresponding fitted values.

- **1.5. Bivariate Regression With Dummy Regressor**: with dummy off: 

\[ \mathbb{E}(Y_i|Z_i = 0) = \alpha \]

\[ \mathbb{E}(Y_i|Z_i = 1) - \mathbb{E}(Y_i|Z_i = 0) = \alpha + \beta \]

So $\beta$ = $\mathbb{E}(Y_i|Z_i = 1) - \mathbb{E}(Y_i|Z_i = 0)$. So if we want a complete definition of expectation of $Y_i$ given dummy $Z_i$, we have:

\[ \mathbb{E}(Y_i|Z_i) = \mathbb{E}(Y_i|Z_i = 0) + (\mathbb{E}(Y_i|Z_i = 1) - \mathbb{E}(Y_i|Z_i = 0))Z_i = \alpha + \beta \]

Hence: $\mathbb{E}(Y_i|Z_i)$ is a linear function of $Z_i$, with slope $\beta$ and intercept $\alpha$. Hence regression fits this CEF perfectly. 

- **1.6 Asymptotic OLS Inference**: 

- **1.7 Omitted Variables Bias**: short equals long plus the effect of omitted times the regression of omitted on included. Derive for the bivariate and multivariate cases. 

- **1.8 Bad Controls**: control variables measured later than the treatment and thus may have been affected by it $\rightarrow$ they are outcomes. 

- **1.9 Measurement Error**: Imagine we are running a regression but instead of directly observing one of the independent variables, we observe a slightly different variable: **measurement error**! **Classical Measurement Errors**: **i)** $\mathbb{E} (m_i) = 0$; **ii)** Cov($S_i^\ast, m_i$) = Cov($e_i,m_i$) = 0. 

If we do the $\beta_b = \frac{Cov(Y_i,S_i)}{V(S_i)}$ calculations, we arrive at $\beta_b = \beta \frac{V(S_i^\ast)}{V(S_i^\ast) + V(m_i)}$. We also get that $r = \frac{V(S_i^\ast)}{V(S_i^\ast) + V(m_i)}$, where $r$ is the **reliability** of the wrong measurement ($S_i$), the variation of $S_i$ that is unrelated to mistakes. The **attenuation bias** in $\beta_b$ is $\beta_b - \beta = -(1 - r)\beta$. So the absolute value of the biased estimator is smaller than the desired one, hence attenuation bias.

- **1.9.2 IV Eliminates Measurement Error in Bivariate Regression**

- If we do the regression anatomy calculations but now using an instrument for $S_i$ when $S_i = S_i^\ast + m_i$, we see that there's no measurement error. 

- **1.9.3 Multivariate**: finish this.

Intuition: adding covariates to a model with mismeasured schooling aggravates attenuation bias in estimates of the returns to schooling. Covariates are correlated with accurately measured schooling while being unrelated to the measurement error.

### **2) Conditional Independence Assumptions**

- Cov(X,Y) = $\mathbb{E}$ ((X - $\mathbb{E}$(X))Y); **intuition**: you only need to difference *one* of the r.v.s when calculating covariance.

- **2.1 Matching Meets Regression**

**Matching**: conditional on $X_i$, treatment $D_i$ is as good as randomly assigned: $\{ Y_{0i},Y_{1i} \perp D_i|X_i \}$. This leads to the Treatment on the Treated being:

\[ \delta_{TOT} = \mathbb{E}(Y_{1i} - Y_{0i}|D_i = 1) \]

Then we LIE on $X_i$

\[ = \mathbb{E}(\mathbb{E}(Y_{1i}|X_i, D_i = 1)- \mathbb{E}(Y_{0i}|X_i, D_i = 1|D_i = 1) \]

math....

Intuition: $\delta_{TOT}$ is a *treatment-probability (conditional probability) of treatment weight average of* $\delta_{X}$, whose sample analogue is observable-sample mean between the treated and the control.

**Conditional Expectation**: $E(X|H) = \frac{E(1_H X)}{P(H)} = \int_X x dP (x|H)$. 

**Regression Meets Matching**: Think of regression estimates parameter $\delta_R$ in $Y_i = \sum_x d_{ix} \beta_x + \delta_R D_i + \epsilon_i$ math... $ = \frac{E[\sigma_D^2 (X_i) \delta_X]}{E[\sigma_D^2 (X_i)]}$. **Intuition**: $\delta_R$ is a *conditional-variance weighted(condition variance of treatment) average of* $\delta_X$, whose sample analogue is observable-sample mean between the treated an the control. In particular,

\[ \sigma_D^2 = P(D_i = 1|_i)[1 - P(D_i = 1|X_i)] \]

- **2.2 Propensity Score**

- Given that $\delta_{TOT}$ = $\mathbb{E}(\delta_X | D_i = 1)$, what happens if $X_i$ is many/multi continuous? Cell specific $\delta_X$ does not make sense (requires grouping or parametric assumptions). The **Propensity-Score Theorem** says we only need to control for covariates that affect the probability of treatment.

### **3) Instrument Variables**

- **3.1 Wald Estimator**: use binary instrument $Z_i$ to estimate model with one endogenous regressor and no covariates. 

**Causal regression model**: $Y_i = \rho s_i + \eta_i$.

If $S_i$ is an endogenous regressor, then $\eta_i$ and $S_i$ may be correlated. Suppose $Z_i$ is a binary instrument that equals 1 with probability $p$. Claim: in the causal regression model above, we have that:

\[ \rho = \frac{\mathbb{E}(Y_i|Z_i = 1) - \mathbb{E}(Y_i|Z_i = 0)}{\mathbb{E}(S_i|Z_i = 1) - \mathbb{E}(S_i |Z_i = 0)} \]

**Exclusion restriction**: $\mathbb{E}(\eta|z = 0)$. 

Leads us to: $\mathbb{E}(y|z) = \rho \mathbb{E}(s|z) + \mathbb{E}(\eta | z) = \rho \mathbb{E}(s|z)$

- **3.2 2SLS**: 

- **3.3 Grouped Data and 2SLS**: i) 2SLS using dummy instruments is = GLS on a set of group means; ii) GLS can be understood as a linear combination of all the Wald estimators that can be constructed from pairs of means.

- **3.4 Heterogeneous Effects**:

**LATE Theorem**: assume, $\forall i$: 
...
\[ \frac{\mathbb{E} (Y_i |Z_i = 1) - \mathbb{E}(Y_i|Z_i = 0)}{\mathbb{E}(D_i|Z_i = 1) - \mathbb{E}(D_i |Z_i = 0)} = \mathbb{E}(Y_{1i} - Y_{0i}|D_{1i} > D_{0i}) = \mathbb{E}(\rho_i | \pi_{1i} > 0) \]

Proof: 1) write in terms of potential outcomes; 2) Rewrite the Wald estimator in terms of potential outcomes, using the law of total expectation; 3) Use independence to simplify the Wald denominator; 4) Combine the pieces.

The intuition is: an instrument that is **i)** as good as randomly assigned, **ii)** affects the outcome through a single known channel, **iii)** has a first-stage and **iv)** affects the causal channel of interest in only one direction, can be used to estimate the **average causal effect on the affected group**. 

- **3.4.1 Failures of Monotonicity**: if monotonicity fails, the instrument pushes some people into treatment while pushing others(defiers) out of treatment. Without monotonicity we could have: $D_{1i} - D_{0i} \in \{ 1,0,-1\}$. So it could be that treatment effects are positive to everyone,, but the reduced form is zero because the effects on compliers are canceled out by the effects on the defiers.

- **3.4.2 IV in Randomized Trials**: **Bloom Result**: when there is one-sided non-compliance, IV estimates the effect of the treatment on the treated in a randomized trial.

- **3.5 2SLS Mistakes**

- **3.5.1 Manual 2SLS**: steps: i) estimate the first stage by OLS; ii) plug the fitted values into the second stage equation; iii) estimate second stage equation by OLS: 
...
i) $X_i$ is a set of covariates;

ii) $Z_i$ is a set of excluded instruments.

iii) The first-stage fitted values are $\hat{S}_i = X_i' \hat{\pi}_{10} + \hat{\pi}_{11}' Z_i$
